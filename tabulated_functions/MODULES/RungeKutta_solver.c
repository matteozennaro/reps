...
void RK (int k_num, double *k, double *BETA, double *FC, double *FN)
{
  ...
  #pragma omp parallel for shared(k_num,k,BETA,FC,FN,OM0,OB0,OC0,OX0,OG0,OR0,h,M_nu,tau_reio,ns,As,kmax,N_nu,Neff,wrong_nu,z0,z1,z_beta_in,z_fc_in,z_fn_in,z_output,output_number,bin_beta_in,bin_fc_in,bin_fn_in,bin_out,mode,alpha,Dc_k_z,Dn_k_z,Xc_k_z,Xn_k_z,o) schedule(dynamic)
  for (j = 0; j < k_num; j++)
  {
    double step;
    int i,ic_set,index_out;
